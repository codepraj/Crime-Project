...
$sql1="select * from applied_courses where stud_id='$stud_id' AND course_complete_university_status='0'";
$query=mysql_query($sql1);
$count=mysql_num_rows($query);
if($count==0){
$sql="insert into applied_courses(`stud_id`,`course_id`,`institute_id`) values('$stud_id','$course_id','$iddd')";
$execute_query=mysql_query($sql);
if($execute_query){
echo "<script>alert('You Have Successfully Approved!!');window.location='course_details.php?id=$course_id';</script>";	
}else{
echo mysql_error();	
}
}else{
echo "<script>alert('You Have Already Apply (Student Can take only one course at a time!)');</script>";		
}
