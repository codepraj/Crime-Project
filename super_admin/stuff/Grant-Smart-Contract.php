...
<link rel="shortcut icon" href="assets/images/favicon.png" type="image/x-icon" />    <!-- Favicon -->
  ...
  $obj= new Connections();
  $sql = $obj->db->prepare("select * from  courses");
...
<a href="course_details.php?id=<?php echo $ID; ?>">
 ...
 $sql1 = $obj->db->prepare("select * from  applied_courses where course_id='$ID'");
  $sql1->execute();
	...
	$course_complete_university_status
	=$row1['course_complete_university_status'];	
  $obj= new Connections();
 ...
 <th>
<?php if($university_approve_status==1 && $course_complete_status==1 && $course_complete_university_status==0){echo "
<br><br><a href='certificate-granted.php?mark=$IDCourse' class='btn btn-primary'>Grant <br> Blockchain Certificate</a>
 ";}else if($course_complete_university_status==1){
?>
<br><a href='certificate.php?id=<?php echo $idstudd; ?>&courseid=<?php echo $course_id; ?>' class='btn btn-primary'>  Blockchain Certificate</a>
...
$sql="UPDATE applied_courses SET course_complete university_status='1' where ID='$mark'";
...
window.location='certificate-granted.php';</script>
<?php 	
}else{
echo mysql_error();	
}
