...
$obj= new Connections(); // Create object of class connection for calling the Personal_Details function
 ...
 if(isset($_POST['username']) && isset($_POST['password']))
        ...
        if ($obj->ICheckLogin($name,$password)==TRUE) // Here we call the CheckLogin function for admin login through the object        
        {
            $_SESSION['cemail']=$name;
	...
	$sql = $obj->db->prepare("select * from state_admin where username='$name'");
	$sql->execute();
	while($row = $sql->fetch(PDO::FETCH_ASSOC)) {
		 $_SESSION['state_admin']=$row['username'];
		 $_SESSION['sid']=$row['ID'];
		
	}
	echo "<script>window.location='state/dashboard.php';</script>";
	}
        else
        {
	...
	<div class="container">
		<br><br>
			    <div class="col-md-6 company_ad">
				  <img src="web/images/GCP-IAM-Best-Practices-A-guide-to-IAM-on-Google-Cloud.webp" style="width: 100%;">
				</div>
				<div class="col-md-6">
				  <div class="contact-form">
				  	<h2 style="color:white;">State Admin Login</h2>
