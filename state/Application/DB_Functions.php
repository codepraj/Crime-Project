...
Class Connections {
	...
	public function CheckLogin($name,$password)
	{
		...
		$sql->execute();
	     ...
	     function deleteupdate($id,$table)
    {
 
                $sql = $this->db->prepare("update guest_message SET resolved_status='1' where ID=:id");
                $sql->bindParam(':id', $id);
                $sql->execute();
                if($sql)
                {
	  ...
	  function Update_for_application_verification($id,$table)
    {
                ...
                $sql = $this->db->prepare("UPDATE $table SET company_verification='1' where ID=:id");
                $sql->bindParam(':id', $id);
                $sql->execute();
                if($sql)
                {
...
return 0;
 ...
 function Add_agents_admin($username,$password){
         
$sql = $this->db->prepare("INSERT INTO `agents` (`userid`, `password`) VALUES(:username,:password)");
...
function Add_Course($title,$desc,$Fees,$Duration,$file,$id,$type)
    {
$sql = $this->db->prepare("INSERT INTO `courses` (`course_name`, `course_image`, `college_id`, `university_status`, `course_conent`, `course_duration`, `course_fees`,`type_course`) VALUES (:title,:file,:id,'0',:desc,:Duration,:Fees,:type)");
		$file=date("mdYHis")."-".$file;
		
		$sql->bindParam(':id', $id);

		$sql->bindParam(':Duration', $Duration);
		$sql->bindParam(':Fees', $Fees);
		$sql->bindParam(':title', $title);
