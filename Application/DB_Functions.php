...
Class Connections {
	...
	{
		$sql = $this->db->prepare("select * from agents where userid=:name and password=:password");
			/* execute statement */
	...
	{
		$sql = $this->db->prepare("select * from super_admin where username=:name and password=:password");
			/* execute statement */
		$sql->bindParam(':name', $name);
        $sql->bindParam(':password', $password);
		$sql->execute();
	...
	{
		$sql = $this->db->prepare("select * from state_admin where username=:name and password=:password");
			/* execute statement */
		$sql->bindParam(':name', $name);
        $sql->bindParam(':password', $password);
		$sql->execute();
 ...
 function insert_guest_message($state,$city,$Suspecious_Activity,$file,$current_hash,$previous_hash){
$sql =$this->db->prepare("INSERT INTO `guest_message` (`state`, `city`,`desc`,`img`,`current_hash`,`previous_hash`)VALUES (:state,:city,:Suspecious_Activity,:file,:current_hash,:previous_hash)");

$file=date("mdYHis")."-".$file;

$sql->bindParam(':state', $state);
$sql->bindParam(':city', $city);
$sql->bindParam(':Suspecious_Activity', $Suspecious_Activity);
$sql->bindParam(':file', $file);
$sql->bindParam(':current_hash', $current_hash);
$sql->bindParam(':previous_hash', $previous_hash);
...
return 0;
 ...
 function insert_miners_reg1($state,$city,$Suspecious_Activity,$file){
$sql =$this->db->prepare("INSERT INTO `agent_message` (`state`, `city`,`desc`,`img`)VALUES (:state,:city,:Suspecious_Activity,:file)");
