...
$obj= new Connections(); // Create object of class connection for calling the Personal_Details function
 ...
 if(isset($_POST['username']) && isset($_POST['password']))
        ...
        if ($obj->comCheckLogin($name,$password)==TRUE) // Here we call the CheckLogin function for admin login through the object        
        {
            $_SESSION['comemail']=$name;
	...
	$sql = $obj->db->prepare("select * from company where company_email='$name'");
	$sql->execute();
	while($row = $sql->fetch(PDO::FETCH_ASSOC)) {
		 $_SESSION['comname']=$row['company_name'];
		 $_SESSION['comid']=$row['ID'];
		  $_SESSION['company_email']=$row['company_email'];
		 $_SESSION['company_logo']=$row['company_logo'];
	}
	echo "<script>window.location='company/dashboard.php';</script>";
	}
        else
        {
	...
	<div class="container">
		<br><br>
			    <div class="col-md-6 company_ad">
				  <img src="web/images/656665.jpg" style="width: 100%;">
				</div>
				<div class="col-md-6">
