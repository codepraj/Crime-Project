...
$obj= new Connections(); // Create object of class connection for calling the Personal_Details function
 ...
 if(isset($_POST['username']) && isset($_POST['password']))
        ...
        if ($obj->uniCheckLogin($name,$password)==TRUE) // Here we call the CheckLogin function for admin login through the object        
        {
            $_SESSION['uniemail']=$name;
	...
	$sql = $obj->db->prepare("select * from super_admin where username='$name'");
	$sql->execute();
	while($row = $sql->fetch(PDO::FETCH_ASSOC)) {
		 $_SESSION['uniname']=$row['username'];
		 $_SESSION['uniid']=$row['ID'];
	
	}
	echo "<script>window.location='super_admin/dashboard.php';</script>";
	}
        else
        {
	...
	<div class="container">
		<br><br>
			    <div class="col-md-6 company_ad">
				  <img src="web/images/graduation_web.webp" style="width: 100%;">
				</div>
				<div class="col-md-6">
